...
const Booking: React.FC = () => {
  ...
  // 3D card tilt effect
  useEffect(() => {
    const cards = document.querySelectorAll('.hotel-card, .history-card');
    cards.forEach((card) => {
  ...
  const cancelBooking = (id: string) => {
    ...
    setBookingHistory(
      bookingHistory.map((b) => (b.id === id ? { ...b, status: 'Cancelled' } : b))
    );
    setError(null);
  };
          ...
          <div className="confirmation-modal fixed inset-0 bg-black/80 flex items-center justify-center z-50">
            <div className="bg-black/60 backdrop-blur-lg border border-yellow-500/30 rounded-2xl p-8 max-w-md w-full">
              ...
              <div className="text-gray-300 space-y-2">
                ...
                <p><strong>PNR:</strong> {bookingHistory[bookingHistory.length - 1].id}</p>
                <p><strong>Booking Date:</strong> {bookingHistory[bookingHistory.length - 1].bookingDate}</p>
                ...
                <p><strong>Base Price:</strong> {currency} {(selectedHotel.price * exchangeRates[currency] * (confirmedGuestDetails.roomType === 'Deluxe' ? 1.2 : confirmedGuestDetails.roomType === 'Suite' ? 1.5 : 1)).toLocaleString()}/night</p>
                <p><strong>Nights:</strong> {differenceInDays(new Date(formData.checkOut), new Date(formData.checkIn))}</p>
                <p><strong>Taxes (15%):</strong> {currency} {(selectedHotel.price * exchangeRates[currency] * (confirmedGuestDetails.roomType === 'Deluxe' ? 1.2 : confirmedGuestDetails.roomType === 'Suite' ? 1.5 : 1) * differenceInDays(new Date(formData.checkOut), new Date(formData.checkIn)) * 0.15).toLocaleString()}</p>
                <p className="font-bold"><strong>Total:</strong> {currency} {(selectedHotel.price * exchangeRates[currency] * (confirmedGuestDetails.roomType === 'Deluxe' ? 1.2 : confirmedGuestDetails.roomType === 'Suite' ? 1.5 : 1) * differenceInDays(new Date(formData.checkOut), new Date(formData.checkIn)) * 1.15).toLocaleString()}</p>
              </div>
                  ...
                  {applyHistoryFilters().map((booking) => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <p><strong>PNR:</strong> {booking.id}</p>
                          <p><strong>Hotel:</strong> {booking.hotel.name}</p>
                          <p><strong>Location:</strong> {booking.hotel.location}</p>
                          <p><strong>Booking Date:</strong> {booking.bookingDate}</p>
                          <p><strong>Status:</strong> {booking.status}</p>
                          <p><strong>Total:</strong> {currency} {booking.total.toLocaleString()}</p>
                        </div>
                        <button
                      ...
                      {expandedBookingId === booking.id && (
                        <div className="mt-4 text-gray-300 space-y-2 border-t border-yellow-500/30 pt-4">
                          ...
                          <p><strong>Room Type:</strong> {booking.guestDetails.roomType}</p>
                          <p><strong>Guest Name:</strong> {booking.guestDetails.name}</p>
                          <p><strong>Email:</strong> {booking.guestDetails.email}</p>
                          <p><strong>Phone:</strong> {booking.guestDetails.phone}</p>
                          <p><strong>Payment Method:</strong> {booking.guestDetails.paymentMethod}</p>
                          <p><strong>Special Requests:</strong> {booking.guestDetails.specialRequests || 'None'}</p>
                          <p><strong>Base Price:</strong> {currency} {(booking.hotel.price * exchangeRates[currency] * (booking.guestDetails.roomType === 'Deluxe' ? 1.2 : booking.guestDetails.roomType === 'Suite' ? 1.5 : 1)).toLocaleString()}/night</p>
                          <p><strong>Nights:</strong> {differenceInDays(new Date(booking.checkOut), new Date(booking.checkIn))}</p>
                          <p><strong>Taxes (15%):</strong> {currency} {(booking.total - (booking.hotel.price * exchangeRates[currency] * (booking.guestDetails.roomType === 'Deluxe' ? 1.2 : booking.guestDetails.roomType === 'Suite' ? 1.5 : 1) * differenceInDays(new Date(booking.checkOut), new Date(booking.checkIn)))).toLocaleString()}</p>
