...
const Flights: React.FC = () => {
  ...
  const [ancillaries, setAncillaries] = useState<{ seatSelection: boolean; extraBaggage: boolean }>({
    seatSelection: false,
    extraBaggage: false,
  });
  ...
  const chartData = {
    labels: [subDays(new Date(formData.departureDate), 3), subDays(new Date(formData.departureDate), 2), subDays(new Date(formData.departureDate), 1), new Date(formData.departureDate), addDays(new Date(formData.departureDate), 1), addDays(new Date(formData.departureDate), 2), addDays(new Date(formData.departureDate), 3)].map(date => format(date, 'MMM dd')),
    datasets: [
      {
        label: 'Price (PKR)',
  ...
  return (
    <div className="relative min-h-screen bg-black text-white font-sans overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6 md:p-20">
        ...
        <div className="form-container bg-black/60 backdrop-blur-lg border border-yellow-500/30 rounded-2xl p-8 mt-16 shadow-lg hover:scale-105 transition-transform duration-500 hover:shadow-yellow-500/30">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="relative">
              <input
                ...
                className="p-3 bg-black/50 text-white rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-yellow-400 border border-yellow-500/30"
                aria-label="Origin city"
              />
              {suggestions.length > 0 && formData.origin && (
                <ul className="absolute bg-black/80 text-white rounded-lg mt-1 max-h-40 overflow-auto z-20">
                  {suggestions.map((s) => (
                    <li
                      key={s.code}
                      onClick={() => {
            ...
            <div className="flex items-center justify-center">
              <button
                onClick={handleSwap}
                className="p-2 bg-yellow-400/20 rounded-full hover:bg-yellow-400/40 transition-colors duration-300"
            ...
            <input
              ...
              className="p-3 bg-black/50 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 border border-yellow-500/30"
            ...
            <div className="flex items-center">
              ...
              />
              <label className="text-gray-300">Flexible Dates (±3 days)</label>
            </div>
            <button
              onClick={handleSearch}
              className="col-span-1 md:col-span-4 bg-gradient-to-r from-yellow-400 to-white text-black py-3 rounded-lg hover:bg-yellow-400 transition-colors duration-300 transform hover:scale-105"
              disabled={loading}
        ...
        {formData.flexibleDates && (
          <div className="mt-16 bg-black/60 backdrop-blur-lg border border-yellow-500/30 rounded-2xl p-8">
            <h2 className="text-2xl font-semibold bg-gradient-to-r from-yellow-400 to-white bg-clip-text text-transparent">
              Price Trends
            </h2>
        ...
        <div className="mt-16 flex flex-col md:flex-row gap-4">
          ...
          />
          <select
            onChange={(e) => setFilters((prev) => ({ ...prev, stops: Number(e.target.value) || undefined }))}
            className="p-3 bg-black/50 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 border border-yellow-500/30"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-16">
          {filteredFlights.map((flight) => (
            <Tilt key={flight.id}>
              <div className="flight-card bg-black/60 backdrop-blur-lg border border-yellow-500/30 rounded-2xl p-6 shadow-lg hover:shadow-yellow-500/30 transition-shadow duration-500">
                <div className="flex items-center mb-4">
                  <FaPlane className="text-3xl text-yellow-400 mr-2" />
                  <h2 className="text-2xl font-semibold bg-gradient-to-r from-yellow-400 to-white bg-clip-text text-transparent">
                ...
                <p className="text-gray-300">Duration: {flight.duration}</p>
                <p className="text-gray-300">
                  Stops: {flight.stops === 0 ? 'Non-Stop' : `${flight.stops} Stop(s)`}
                </p>
                <p className="text-gray-300">Class: {flight.cabinClass}</p>
                <button
                ...
                <button
                  ...
                  className="mt-4 bg-gradient-to-r from-yellow-400 to-white text-black py-2 px-4 rounded-lg hover:bg-yellow-400 transition-colors duration-300 transform hover:scale-105"
        ...
        <div className="mt-16 bg-black/60 backdrop-blur-lg border border-yellow-500/30 rounded-2xl p-8">
          ...
          <div className="flex gap-4 mt-4">
            ...
            <button
              ...
              className="bg-gradient-to-r from-yellow-400 to-white text-black py-2 px-4 rounded-lg hover:bg-yellow-400 transition-colors duration-300"
              aria-label="Check flight status"
            >
              Check Status
            </button>
          </div>
          {flightStatus && <p className="mt-4 text-gray-300">{flightStatus}</p>}
        </div>

        {/* Booking Modal */}
        {showModal && (
          <div className="modal fixed inset-0 bg-black/80 flex items-center justify-center z-50">
              ...
              <form onSubmit={handleModalSubmit} className="mt-4 space-y-4">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => setAncillaries((prev) => ({ ...prev, seatSelection: e.target.checked }))}
                    className="mr-2"
                    aria-label="Seat selection"
                  />
                  <label className="text-gray-300">Seat Selection (+PKR 5000)</label>
                </div>
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    checked={ancillaries.extraBaggage}
                    onChange={(e) => setAncillaries((prev) => ({ ...prev, extraBaggage: e.target.checked }))}
                    className="mr-2"
                    aria-label="Extra baggage"
                  />
                  <label className="text-gray-300">Extra Baggage (+PKR 3000)</label>
                </div>
                <div className="flex justify-end gap-4">
                  ...
                  <button
                    type="submit"
                    className="py-2 px-4 bg-gradient-to-r from-yellow-400 to-white text-black rounded-lg hover:bg-yellow-400"
...
// Error Boundary
class FlightsErrorBoundary extends React.Component<{}, { hasError: boolean }> {
  ...
  render() {
    if (this.state.hasError) {
      return <div className="text-center text-red-500 p-10">Something went wrong. Please refresh the page.</div>;
    }
